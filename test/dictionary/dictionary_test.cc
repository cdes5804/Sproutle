...
TEST_CASE("Test New Word Satisfies Rule 1") {
  ...
  Dictionary dictionary(answer_pool_path, allowed_guess_path,
                        MAX_COMMON_CHAR_ALLOWED);
  ...
  for (uint32_t i = 0; i < test_round; i++) {
    ...
    REQUIRE(GetCommonCharacterCount(last_word, word) <=
            MAX_COMMON_CHAR_ALLOWED);
...
TEST_CASE("Test Only One Valid Word") {
  ...
  Dictionary dictionary(temp_file_path, temp_file_path,
                        MAX_COMMON_CHAR_ALLOWED);
